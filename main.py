...
# --- ИЗМЕНЕНИЕ: Добавляем новый импорт для URL-кодирования ---
import urllib.parse
...
# --- ИЗМЕНЕНИЕ: Указываем более новую модель для лучших результатов ---
...
# --- ИЗМЕНЕНИЕ: Обновление промптов для поддержки генерации изображений ---
...
4.  **РАБОТА С ИЗОБРАЖЕНИЯМИ:** Если считаешь, что изображение с тегом `[IMAGE_XXX]` напрямую иллюстрирует твой ответ, добавь этот тег в самый конец своего ответа.
5.  **ЦИТИРОВАНИЕ ИСТОЧНИКОВ:** После основного ответа, ДОБАВЬ разделитель `%%SOURCES%%`. После него перечисли через запятую названия каналов, из которых была взята информация.
6.  **ГЕНЕРАЦИЯ ИЗОБРАЖЕНИЙ ДЛЯ СОБЫТИЙ (ВАЖНО!):**
    - Если твой ответ содержит информацию из раздела "СВОДКИ АКТУАЛЬНЫХ СОБЫТИЙ И СПЛЕТЕН", ты **ОБЯЗАН** создать промпт для генерации изображения по этому событию.
    - После разделителя `%%SOURCES%%` добавь еще один разделитель: `%%IMAGE_PROMPTS%%`.
    - После `%%IMAGE_PROMPTS%%` напиши с новой строки краткий, но яркий промпт на английском языке, описывающий сцену события. **Только сцену!** Не добавляй в промпт слова "scroll", "ink", "drawing". Только описание действия.
    - Если событий несколько, создай несколько промптов, каждый с новой строки.
    - Пример: `%%IMAGE_PROMPTS%%\nA hooded figure nailing a wanted poster to a medieval town's message board at night.\nA broken merchant cart with a shattered wheel on a muddy forest road.`
...
1.  **ОСНОВА — ЛОР, НО НЕ КОПИПАСТА:** Твои ответы всегда опираются на предоставленный текст. Но не пересказывай его дословно. Выжимай суть.
2.  **ДОДУМЫВАЙ, НО ВНУТРИ ЛОРА:** Если прямого ответа нет, ты можешь сделать циничное предположение, но оно должно быть основано ИСКЛЮЧИТЕЛЬНО на других фактах из лора.
3.  **МАТ И САРКАЗМ — ТВОИ ИНСТРУМЕНТЫ:** Используй маты и черный юмор.
...
5.  **КАРТИНКИ:** Если картинка с тегом `[IMAGE_XXX]` подходит, вставь этот тег в конец.
6.  **РИСУНКИ СПЛЕТЕН (СЛУШАЙ СЮДА):**
    - Твой магический артефакт иногда показывает образы. Если отвечаешь на что-то из "АКТУАЛЬНЫХ НОВОСТЕЙ", ты **ДОЛЖЕН** описать этот образ для художника.
    - После разделителя `%%SOURCES%%` добавь `%%IMAGE_PROMPTS%%`.
    - После `%%IMAGE_PROMPTS%%` с новой строки коротко, на английском, опиши, что ты видишь. **Только саму сцену!** Без слов "свиток", "чернила" и прочей лабуды.
    - Если новостей несколько, опиши каждую картинку с новой строки.
    - Пример: `%%IMAGE_PROMPTS%%\nA shadowy figure slipping a letter under a noble's door in a dark alley.\nTwo city guards arguing over a spilled crate of fish in a bustling market.`
...
# --- ИЗМЕНЕНИЕ: Новая функция для генерации изображений через Pollinations.ai ---
async def generate_pollinations_image(session: aiohttp.ClientSession, description_prompt: str) -> bytes | None:
    ...
    try:
        # Формируем полный промпт, добавляя стилистические требования
        ...
        width = 1024
        height = 768  # Соотношение сторон, более подходящее для свитка
        seed = random.randint(1, 100000) # Случайный seed для разнообразия
        model = 'flux' # Используем модель, указанную в примере
        
        image_url = f"https://pollinations.ai/p/{encoded_prompt}?width={width}&height={height}&seed={seed}&model={model}"

        # Асинхронно запрашиваем изображение
        async with session.get(image_url, timeout=120) as resp: # Увеличим таймаут
            if resp.status == 200:
...
async def ask_lore(interaction: discord.Interaction, question: str, personality: discord.app_commands.Choice[str] = None):
    ...
    try:
        ...
        # --- ИЗМЕНЕНИЕ: Парсинг ответа на основной текст, источники и промпты для изображений ---
        files_to_send = []
        image_prompts_text = ""
        
        # Сначала отделяем промпты для изображений, если они есть
        if "%%IMAGE_PROMPTS%%" in raw_text:
            parts = raw_text.split("%%IMAGE_PROMPTS%%")
            raw_text = parts[0]
            image_prompts_text = parts[1].strip()

        # Затем отделяем источники
        answer_text, sources_text = (raw_text.split("%%SOURCES%%") + [""])[:2]
        answer_text = answer_text.strip()
        sources_text = sources_text.strip()

        # Поиск тегов для уже существующих изображений в лоре
        ...
        # Создание и отправка основного ответа
        ...
        # Отправка существующих изображений из лора, если они есть
        if files_to_send:
        ...
        # --- ИЗМЕНЕНИЕ: Генерация и отправка новых изображений по событиям ---
        if image_prompts_text:
            ...
            async with aiohttp.ClientSession() as session:
                for i, img_prompt in enumerate(image_prompts):
                    ...
                    if image_bytes:
                        # Превращаем байты в файл Discord "на лету"
